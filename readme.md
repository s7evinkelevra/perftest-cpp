...
The `module unload env/system-gcc` is used to unload the default gcc compiler before loading a newer version. I recommend adding these commands to the .profile or .bashrc file in your user folder to load them automatically when you connect to hummel.

## Generating the build files
Once the source code is downloaded and everything is installed on the system, we can generate the build files using cmake.

### All Platforms
1. Navigate to the folder of the downloaded source code.
...
You can define SLURM jobs using a bash script, with many options to set up your job using the simulation model. Here, I will briefly go through the script I used to feed each instance a different configuration file. It requires a certain setup, but worked very nicely for my purposes.

For it to work, you need the following folders and files:
- The script below somewhere in your `$HOME` folder.
